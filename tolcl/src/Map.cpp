...
{
    WorldObjNode::WorldObjNode( PickingListener* pickingListener, const String& name, const Vector<float>& loc, float orientation, Model* model )
    ...
    const Vector<float>& WorldObjNode::getLoc()
    {
        return loc;
    }
    ...
    Sector::Sector( Map* map, unsigned sx, unsigned sy )
            : map( map ), sx( sx ), sy( sy )
    {
        String fileName = ( String )"tolcl/area/" + sx + "+" + sy + ".sect";

        InputStream* sectorFile = sg->open( fileName );
    ...
    {
        InputStream* objectInfo = sg->open( ( String )"tolcl/world/" + name + ".obj" );
        ...
        else if ( type == "model" )
        {
            ...
            Model* model = globalResMgr->getModel( source );
    ...
    void Sector::save()
    {
        File output( ( String )"tolcl/area/" + sx + "+" + sy + ".sect", true );
    ...
    Map::Map( SceneGraph* graph, PickingListener* pickingListener, float xc, float yc )
            : pickingListener( pickingListener ), csx( -1 ), csy( -1 )
    {
    ...
    void Map::addReference( unsigned wmid )
    {
        ...
        InputStream* meshInfo = sg->open( ( String )"tolcl/world/" + wmid + ".mesh" );
        ...
        if ( type == "heightmap" )
        {
            ...
            HeightMap* height = new HeightMap( source, Vector<float>( w, h, range ), z0 );
            Model* model = Model::createTerrain( height, Vector<float>( w, h ) / 5.0f, Material::createSimple( texture ) );
    ...
    float Map::getHeightAt( float x, float y )
    {
        iterate ( terrains )
        {
            ...
            if ( x >= curr.x && y >= curr.y && x < curr.x + curr.w && y < curr.y + curr.h )
                return curr.height->get( x - curr.x, y - curr.y );
        }
    ...
    {
        SG_assert( mx < 5, "GameClient.Map", "load" )
        SG_assert( my < 5, "GameClient.Map", "load" )
    ...
    void Map::unlock( const Vector<float>& playerLoc )
    {
        world->leave();
        world->reset();
